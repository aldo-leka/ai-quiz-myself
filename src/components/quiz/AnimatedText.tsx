...
export function AnimatedText({ text, onComplete, onCue }: AnimatedTextProps) {
  ...
  return (
    <button
      ...
      onClick={onTap}
      className={cn(
        "w-full rounded-2xl border border-[#252940] bg-[#1a1d2e]/78 p-4 text-left text-[clamp(1.25rem,5.4vw,3.25rem)] leading-tight text-[#e4e4e9] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#818cf8]/70 md:p-6",
        compactLayout && "md:p-4 md:text-[clamp(1.15rem,2.4vw,2rem)]",
        tvLikeLayout && "md:text-[clamp(1.7rem,3.6vw,3rem)]",
      )}
    >
      ...
      <span
        className={cn(
          "mt-3 block text-sm text-[#9394a5] md:mt-4 md:text-lg",
          compactLayout && "md:mt-3 md:text-base",
        )}
      >
