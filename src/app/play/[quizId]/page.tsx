...
export default function PlayQuizPage() {
  ...
  useEffect(() => {
    ...
    async function loadQuiz() {
      ...
      try {
        ...
        await welcomePlayer(payload.quiz);
      } catch (error) {
    ...
    // This load cycle should only run when the route quiz id changes.
  ...
  async function persistSession(score: number) {
    if (!quiz || !sessionData?.user || hasPersistedSessionRef.current) return;
