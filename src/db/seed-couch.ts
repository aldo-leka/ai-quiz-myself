...
function createPrompt(theme: string): string {
  return `You are a senior quiz writer creating a couch co-op trivia round for families.
...
- Plausible distractors; avoid joke answers.
- Explanations must teach something useful.
- Difficulty should ramp gently:
  - About 1/3 easy
  - About 1/3 medium
  - About 1/3 hard
- Keep topics varied within the theme.
- Avoid repeating the same wording patterns.
...
async function main() {
  ...
  console.log(`Seeding ${THEMES.length} couch co-op hub quizzes with model: ${modelName}`);
